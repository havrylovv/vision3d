...
from vision3d.utils.build import build_loss, build_model
...
class MonoDETR3D(Vision3DModel):
    ...
    def prepare_inputs_to_fusion_transformer(
        self, point_cloud: Tensor, image_features: List[Tensor], pc_features: List[Tensor]
        ...
        image_masks = [
            torch.zeros((B, *feat_shape), dtype=torch.bool, device=missing_mask.device)
            for feat_shape in features_shapes
        ]
        ...
        pc_inputs = {"srcs": pc_features, "masks": pc_masks, "pos_embeds": pc_pos_embeds}

        return image_inputs, pc_inputs

    def generate_position_embeddings(
        self, features: List[Tensor], masks: List[Tensor], position_embedding_module
    ) -> List[Tensor]:
        """
