...
class SpatiallyAwareTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.cross_attn_blocks = nn.ModuleList(
            [
                CrossAttentionBlock(d_model, nhead, dim_feedforward, dropout, activation)
    ...
    def forward(
        ...
        memory1 = self.memory_pool(
            memory1.permute(0, 2, 1), size=self.memory_pool_dim, mode="linear", align_corners=False
        ).permute(0, 2, 1)
        memory2 = self.memory_pool(
            memory2.permute(0, 2, 1), size=self.memory_pool_dim, mode="linear", align_corners=False
        ).permute(0, 2, 1)
        ...
        queries = self.query_embed.unsqueeze(1).repeat(
            1, fused_memory.size(0), 1
        )  # (num_queries, batch_size, query_dim)
